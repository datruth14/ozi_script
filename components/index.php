<!-- calling custome cssd -->
<?php
 styleSheet('home/index');
?>

<?php navBar(); ?>

<div class="px-4 py-5 my-5  text-center">
    <img src="assets/media/images/icons/android-chrome-192x192.png" alt="OziApp" class="lazy">
    <h1 class="display-5 fw-bold animate__animated animate__rubberBand animate__delay-2s	2s animate__infinite animate__slower 3s" style="color:#020238;">Congratulations!</h1>
    <div class="col-lg-6 mx-auto">
      <p class="lead mb-4" style="color:purple;font-weight:430;">You have successfully downloaded and installed ozi script project structure, Let's start by editing the index.php component inside your project directory. <a href="#"> Learn More </a></p>

      <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
         <a  id="getbtn" href="#"  class="btn  btn-lg px-4 gap-3"> Get Started With Widgets  </a>      
      </div>

    </div>
</div>